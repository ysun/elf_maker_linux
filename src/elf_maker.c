...
elf_file_t *elf_maker_init()
{
  ...
  header.e_ident[EI_CLASS] = ELFCLASS64;   /* architecture version (64bit) */
  ...
  header.e_shentsize = sizeof(Elf64_Shdr); /* size of entry in the section header table */
...
elf_section_t *elf_maker_add_section(elf_file_t *elf_file, char *name, Elf64_Addr vaddr)
{
  /* error checking */
  if (!elf_file)
    return NULL;

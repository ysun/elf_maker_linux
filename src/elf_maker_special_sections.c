...
elf_section_t *add_null_section(elf_file_t *elf_file)
{
  if (!elf_file)
    return NULL;
  elf_section_t *null_section = elf_maker_add_section(elf_file, NULL, -1);
  return null_section;
}
...
elf_section_t *add_strings_section(elf_file_t *elf_file)
{
  ...
  elf_section_t *strings_section = elf_maker_add_section(elf_file, ".shstrtab", -1);
...
elf_section_t *add_symbol_table_section(elf_file_t *elf_file)
{
  ...
  elf_section_t *symbol_table_section = elf_maker_add_section(elf_file, ".symtab", -1);
...
elf_section_t *add_symbol_table_strings_section(elf_file_t *elf_file, elf_section_t *symbol_table_section)
{
  ...
  elf_section_t *symbol_table_strings_section = elf_maker_add_section(elf_file, ".strtab", -1);
...
elf_section_t *add_text_section(elf_file_t *elf_file, elf_section_t *symbol_table_section, elf_section_t *symbol_table_strings_section, Elf64_Addr vaddr)
{
  ...
  /* add section */
  elf_section_t *text_section = elf_maker_add_section(elf_file, ".text", vaddr);
